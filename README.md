...
标签是VAPA提案管理的核心基础设施。请在GitHub仓库中严格按照以下规范创建标签。

#### 提案类型 `type:`

| 标签 | 颜色 | 含义 |
|------|------|------|
| `type: vision-amendment` | `#6f42c1` | 对VISION.md的修正提案 |
| `type: feature` | `#0075ca` | 新能力提案 |
| `type: problem` | `#e4e669` | 纯问题陈述，不含解法 |
| `type: improvement` | `#a2eeef` | 对现有能力的改善 |
| `type: experiment` | `#d93f0b` | 假设验证型提案 |
| `type: technical-debt` | `#e99695` | 技术债清理 |
| `type: research` | `#f9d0c4` | 需要调研后才能形成提案 |
...
| 标签 | 颜色 | 含义 |
|------|------|------|
| `align: core` | `#0075ca` | 直接支撑当前战略重心 |
| `align: adjacent` | `#a2eeef` | 相邻，有间接价值 |
| `align: exploratory` | `#f9d0c4` | 探索性，超出当前重心 |
| `align: off-track` | `#b60205` | 偏离方向，需专项讨论 |

#### 规模估计 `size:`

| 标签 | 颜色 | 参考标准 |
|------|------|---------|
| `size: S` | `#0e8a16` | Agent可在1天内独立完成 |
| `size: M` | `#fbca04` | Agent需2–5天，需人工拆解辅助 |
| `size: L` | `#e4e669` | 必须拆分为多个子提案执行 |
| `size: XL` | `#b60205` | 战略级，需专项讨论后再拆解 |

#### 贡献角色 `contrib:`

| 标签 | 说明 |
|------|------|
| `contrib: proposer` | 提案发起者（Issue创建时自动标记） |
| `contrib: shaper` | 实质性完善贡献者 |
| `contrib: reviewer` | 正式评审参与者 |
| `contrib: validator` | 验收执行者 |
| `contrib: sponsor` | 提案战略背书人 |
...
|------|------|------|
| `Proposer` | Text | 提案发起人 |
| `Shapers` | Text | 实质贡献的完善者 |
| `Validator` | Text | 验收执行者 |
| `Contribution Score` | Number | 见第六章 |
| `Strategic Alignment` | Single Select | core / adjacent / exploratory |
